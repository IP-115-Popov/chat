...
import com.eltex.chat.data.mappers.LoginResponseToUserMapper
...
class SignInRepositoryImpl @Inject constructor(
    ...
    override suspend fun signIn(loginUiRequest: LoginUiModel): Either<SignInError, User> {
        val response =
        ...
        return if (response.isSuccessful) {
            ...
            if (loginResponse != null) {
                LoginResponseToUserMapper.map(loginResponse).right()
            } else {
                when (response.code()) {
